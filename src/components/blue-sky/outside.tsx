...
const Outside = ({ name, date }: { name?: string; date?: string }) => {
    return (
        <div className='w-full flex-col lg:flex justify-between hidden bg-no-repeat h-screen bg-[#629BC033] sticky top-0'>
            ...
            <div className="flex flex-col items-center justify-center">
                <p className='text-[#212121] font-allison text-[50px]'>{name || "Jane & John"}</p>
                <p className='text-[#212121] font-medium flex items-center'><span className='size-[8px] mr-2 rounded-full bg-[#212121] block'></span> {date || "20.01.2026"} <span className='size-[8px] ml-2 rounded-full bg-[#212121] block'></span></p>
            </div>
